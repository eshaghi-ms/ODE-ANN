...
$\quad\quad\quad\quad M=1 \quad kg$
$\quad\quad\quad\quad D=0.05 \quad kg·s$
$\quad\quad\quad\quad K(x)=1+0.1x \quad N/m$
$\quad\quad\quad\quad F_e = F·sin(ωt)  \quad N$
...
![Figure 1](Figuers/1.png)

By inserting the values of the given terms, the following differential equation is obtained:

$$\frac{d^2x}{dt^2} + 0.05\ \frac{dx}{dt} + (1 + 0.1\ x)\ x = F\sin(\omega t) \quad\quad\quad\quad (1)$$
...
$$ x(0)=0, \quad	x'(0)=0  \quad\quad\quad\quad (2)$$
...
**Fig. 1** the stable behavior of non-linear mass-spring-damper system ($F=5$ and $ω=5$): (a) displacement of mass (meters), as a function of the time (second); (b) velocity of mass, $dx/dt$, ($m/s$), as a function of its displacement.

(a)![](Figuers/4.jpg)

(b)![](Figuers/5.jpg)

**Fig. 2** the unstable behavior of non-linear mass-spring-damper system ($F=0.8$ and $ω=0.81$): (a) displacement of mass (meters), as a function of the time (second); (b) velocity of mass, $dx/dt$, ($m/s$), as a function of its displacement.
...
In this way, a database of two inputs ($ω$ and $F$)  and one output ($R_{max}$) with $74,451$ rows of data was provided for developing neural networks.
...
MLP is a class of ANNs, and stands as an evolution of the perceptron neural network originally developed in the 1960s, see (Noriega 2005). MLP provides a nonlinear map $f:R^k⟶R^h$, in case of an input layer made of $k$ neurons (input values) and an output layer made of $h$ neurons (output values); it also consists of an arbitrary number of hidden layers, with a variable number of neurons. The neurons are storage cells for scalar values, obtained by an activation function applied to the neuron values in the previous layer.
...
$$RMSE = \sqrt{\frac{1}{n} \sum_{i=1}^{n} (R_{m_i, D} - R_{m_i, ANN})^2} \quad\quad\quad\quad (4)$$

which has been assumed as the loss function to be minimized during the training. In Eq. ($4$): $n$ is the total number of data; for the $i$-th model, $R_{m_i , D}$ is the maximum restoring load obtained with the MATLAB software (in Database) while $R_{m_i ,  ANN}$  is the value estimated through the ANN algorithm. 
...
$$R^2 = \left[\sum_{i=1}^{n} (R_{m_i, D} - \overline{R_{m, D}})(R_{m_i, ANN} - \overline{R_{m, ANN}}) \     /  \     \sqrt{\sum_{i=1}^{n} (R_{m_i, D} - \overline{R_{m, D}})^2 \sum_{i=1}^{n} (R_{m_i, ANN} - \overline{R_{m, ANN}})^2}   \right]^2 \quad\quad\quad\quad (5)$$

$$SI = \frac{RMSE}{\overline{R_{m, D}}}\quad\quad\quad\quad (6)$$
